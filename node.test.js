...
(function ($) {
    class $hyoo_sync_server extends $hyoo_sync_yard {
        http() {
            const server = $node.http.createServer($mol_wire_async((req, res) => {
                ...
                function proceed(data, node, query) {
    ...
    if (port)
        $mol_wire_async($hyoo_sync_server).run(port);
})($ || ($ = {}));
...
(function ($) {
    ...
    $mol_test({
        async 'import exported html'() {
            ...
            $mol_assert_equal(left.chief.as($hyoo_crowd_text).text(), right.chief.as($hyoo_crowd_text).text(), 'foobar');
        },
